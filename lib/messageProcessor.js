...
function resolveAutoResponse(detection, cliProfile = null) {
  ...
  const patternName = detection?.patternName || "";
  
  // Use CLI-specific auto-response mapping if available
  if (cliProfile && cliProfile.autoResponses) {
    const responses = cliProfile.autoResponses;
    
    if (patternName.includes("continue") || patternName.includes("confirm")) {
      return responses.continue || responses.confirm || "y";
    } else if (patternName.includes("yes_no")) {
      return responses.confirm || "y";
    } else if (patternName.includes("press_enter")) {
      return responses.press_enter || "";
    } else if (patternName.includes("selection")) {
      return responses.select || "1";
    } else if (patternName.includes("approval")) {
      return responses.confirm || "y";
    }
  }
  
  // Fallback to generic logic
  const prompt = analyzePrompt(excerpt, patternName);
  if (prompt.rateLimitOption) return prompt.rateLimitOption.key;

  const looksRateLimited = RATE_LIMIT_PATTERN_NAMES.has(patternName)
  ...
  if (prompt.hasPressEnterPrompt || patternName === "press_enter") return "";
