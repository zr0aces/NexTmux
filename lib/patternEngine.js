...
function convertCliProfilePatterns(profile) {
  // Convert CLI profile patterns object to standard array format
...
function createPatternEngine({ patterns = DEFAULT_PATTERNS, cliType = null, cliProfile = null, linesToInspect = 120 } = {}) {
  // Determine patterns to use: explicit patterns > CLI profile > defaults
  let patternsToUse = patterns;
  
  // Only try CLI profile if patterns were not explicitly provided
  if (patterns === DEFAULT_PATTERNS && cliProfile) {
    const profilePatterns = convertCliProfilePatterns(cliProfile);
    if (profilePatterns && profilePatterns.length > 0) {
      patternsToUse = profilePatterns;
    }
  }
  
  const compiled = (Array.isArray(patternsToUse) ? patternsToUse : DEFAULT_PATTERNS)
    .map(toSafePattern)
    .filter(Boolean);
  ...
  function detect(output) {
    ...
    for (const item of compiled) {
      ...
      for (let i = excerptLines.length - 1; i >= 0; i--) {
        if (excerptLines[i].toLowerCase().includes(matchedText.toLowerCase())) {
          matchedLine = excerptLines[i].trim();
          break;
        }
      }
      if (!matchedLine) {
...
function getIanaTzOffsetMin(tzName, date = new Date()) {
  try {
    const format = (tz) => {
      ...
      let hr = parseInt(dict.hour, 10);
      const isMidnightEnd = hr === 24;
      if (isMidnightEnd) hr = 0;
      
      const ms = Date.UTC(
        parseInt(dict.year, 10),
        parseInt(dict.month, 10) - 1,
        parseInt(dict.day, 10),
        hr,
        parseInt(dict.minute, 10),
        parseInt(dict.second, 10)
      );
      return isMidnightEnd ? ms + 86400000 : ms;
    };
    const tzMs = format(tzName);
    const utcMs = format("UTC");
...
function parseResetEpoch(text, now = Date.now()) {
  ...
  // Absolute: "11:00 AM PST", "15:30 UTC", "3:45 PM", "12:20am (Asia/Bangkok)"
  const absRe = /(\d{1,2}):(\d{2})(?::(\d{2}))?(?:\s*(am|pm))?(?:\s+((?:GMT|UTC)[+-]\d+(?::\d+)?|UTC|GMT|EST|EDT|CST|CDT|MST|MDT|PST|PDT|\(?[A-Za-z_]+\/[A-Za-z_]+(?:\/[A-Za-z_]+)?\)?))?/i;
  const absMatch = absRe.exec(s);
  if (absMatch) {
    let h = parseInt(absMatch[1], 10);
    const min = parseInt(absMatch[2], 10);
    const ampm = (absMatch[4] || "").toUpperCase();
    const tzRaw = absMatch[5] || "";
    const cleanTz = tzRaw.replace(/[()]/g, "").trim();
    const tzStr = cleanTz.toUpperCase();
    ...
    } else if (/\//.test(cleanTz)) {
      const ianaOff = getIanaTzOffsetMin(cleanTz, new Date(now));
      if (ianaOff !== null) {
        tzOffMin = ianaOff;
      }
    } else {
      const gmtMatch = /^(?:GMT|UTC)([+-])(\d+)(?::(\d+))?$/.exec(tzStr);
      if (gmtMatch) {
    ...
    let targetMs = todayAtHM - tzOffMin * 60000;
    while (targetMs <= now) {
      targetMs += 86400000;
    }
    return targetMs;
