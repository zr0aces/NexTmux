function createWatcherEngine({ patternEngine, messageProcessor, options = {} } = {}) {
  const enabled = options.enabled !== false;
  const idleThresholdMs = Math.max(1000, Number(options.idleThresholdMs) || 5000);
  const detector = messageProcessor && typeof messageProcessor.detect === "function"
    ? messageProcessor
    : patternEngine;
  ...
  function inspect({ output, previousOutput, currentState, lastChangeTime, now = Date.now() }) {
    ...
    const detection = detector?.detect
      ...
      : { matched: false, patternName: null, matchedText: "", excerpt: "", detectedAt: null, autoResponse: null };
...
function getNewLinesCount(currentOutput, previousOutput) {
  ...
  const maxSearch = Math.min(prevLines.length, currLines.length, 100);
