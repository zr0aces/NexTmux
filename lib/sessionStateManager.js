...
function createSessionStateManager({ notifyCooldownMs = 120000, stateFilePath } = {}) {
  ...
  function shouldNotify({ worker, patternName, matchedText, excerpt, now = Date.now() }) {
    const sessionKey = getKey(worker) || "unknown";
    const key = `${sessionKey}::${patternName || "unknown"}::${hashText((matchedText || "") + "\n" + (excerpt || ""))}`;
  ...
  function markNotification(worker, status, now = Date.now(), key = null) {
    ...
    if (status === "sent" && key) {
      ...
      lastNotifyBySession.set(sessionKey, now);
    }
  ...
  function removeSession(worker) {
    ...
    const sessionKey = getKey(worker) || "unknown";
    lastNotifyBySession.delete(sessionKey);
