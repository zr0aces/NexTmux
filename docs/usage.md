...
- **Mechanical Toolkit (Keyboard Icon)**: Click the **⌨** keyboard button to reveal standard key shortcuts (`esc`, `↑`, `↓`, `↵`, `tab`, `⇧tab`, `⌃c`) plus quick command buttons (`proceed`, `continue`, `yes`) for fast manual interaction. On mobile, this expands to a full-width toolbar dock docked neatly at the bottom of the screen. The keyboard layout is optimized for ergonomic right-hand reach, placing the most frequent controls (arrows, enter, proceed, and yes) on the right side of the grid.
